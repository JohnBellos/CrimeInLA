...
join1 = crime_vics.join(rev_geo.hint("BROADCAST"),on=["LAT", "LON"], how="inner")
join1.explain()
...
join2 = income.join(join1.select("Zip Code").hint("BROADCAST"), on="Zip Code", how="inner")
join2.explain()
...
top3 = join1.join(top3zip.hint("MERGE"), on=["Zip Code"], how="inner")
bottom3 = join1.join(bottom3zip.hint("MERGE"), on=["Zip Code"], how="inner")
top3.explain()
bottom3.explain()
